...
/// The six user-supplied image parameters from which all other constants are derived.
#[derive(Debug, Clone)]
...
impl ImageParams {
    ...
    /// Base B for garbage chunk encoding = image_width + 1.
    pub fn garbage_b(&self) -> usize {
        self.image_width + 1
    }

    /// Number of garbage blocks per row = image_width.
    ...
    /// Number of garbage column words = ceil(image_width / 8).
    ...
    /// Max blocks per well-behaved chunk = min(ceil(L / (image_width / wb_nb)), L).
    pub fn wb_m(&self) -> usize {
        let pixels_per_module = self.image_width as f64 / self.wb_nb() as f64;
    ...
    pub fn to_params_zok(&self) -> String {
        ...
        out.push_str(&format!(
            "// Parameters: image_width={}, image_height={}, max_rows={}, max_cols={}, max_ec_level={}, chunk_size={}\n\n",
            self.image_width, self.image_height, self.max_rows, self.max_cols, self.max_ec_level, l
        ));
