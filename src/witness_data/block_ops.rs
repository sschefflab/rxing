...
pub fn get_word_6(widths: &[u32; 6]) -> u64 {
    widths
        .iter()
        .enumerate()
...
pub const START_WORD_6: u64 = 804475;
/// Width-word (base-13) for the PDF417 stop pattern [7,1,1,3,1,1,1,2] → widths [8,2,4,4,2,2]
pub const STOP_WORD_6: u64 = 809206;

fn decodes_exactly_6(widths: &[u32; 6]) -> bool {
    let word = get_word_6(widths);
    word != 0 && WIDTH_TO_CODEWORD.get(&word).is_some_and(|&cw| cw != DUMMY_CW)
}

fn ext_codeword_of_width(word: u64) -> u32 {
    if word == 0 || word == START_WORD_6 || word == STOP_WORD_6 {
        return DUMMY_CW;
    }
    *WIDTH_TO_CODEWORD
        .get(&word)
...
/// Computes raw words from normalized widths: each [u32; 6] → u64 via get_word_6.
pub fn compute_words_6(normalized_widths: &[Vec<[u32; 6]>]) -> Vec<Vec<u64>> {
    normalized_widths
        .iter()
        .map(|row| row.iter().map(|w| get_word_6(w)).collect())
        .collect()
}

/// [u32; 6] variant of compute_words_with_dummies.
/// Uses base-13 width-word encoding and table-based validity check.
pub fn compute_words_with_dummies_6(
        ...
        .map(|(i, row)| {
            ...
            if is_consecutive_duplicate {
                ...
                row.iter()
                    ...
                    .map(|(j, widths)| {
                        let word = get_word_6(widths);
                        if word == START_WORD_6 || word == STOP_WORD_6 || decodes_exactly_6(widths) {
                            word
                        } else {
...
pub fn compute_garbage_word_witnesses_6(
    normalized_widths: &[Vec<[u32; 6]>],
) -> (Vec<u64>, Vec<u64>) {
    normalized_widths
        .iter()
        .map(|row| {
            row.iter()
                ...
                .find(|(_, widths)| {
                    let word = get_word_6(widths);
                    word != START_WORD_6 && word != STOP_WORD_6 && !decodes_exactly_6(widths)
                })
                .map(|(idx, widths)| (get_word_6(widths), idx as u64))
                .unwrap_or_else(|| {
            ...
            row.chunks_exact(8)
                .map(|window| {
                    if window.iter().any(|&x| x == 0) {
                        ...
                        let mut widths = [0; 6];
                        for i in 1..7 {
                            widths[i - 1] = window[i] + window[i - 1];
                        }
                        widths
                    }
                })
...
mod tests {
    ...
    #[test]
    fn test_normalized_widths_last_element_excluded() {
        // The last element (index 7) does not appear in any pairwise sum.
        // Changing it from 8 to 99 must not change any output width.
        let blocks_a = vec![vec![1u32, 2, 3, 4, 5, 6, 7, 8]];
        let blocks_b = vec![vec![1u32, 2, 3, 4, 5, 6, 7, 99]];
        assert_eq!(
    ...
    fn test_normalized_widths_multiple_chunks() {
        // 16 elements → 2 chunks, each producing 6 widths independently.
        let blocks = vec![vec![
            1u32, 1, 1, 1, 1, 1, 1, 1, // chunk 0: all 1s
            2, 3, 4, 5, 6, 7, 8, 9,    // chunk 1: ascending
        ]];
        let widths = compute_normalized_widths(&blocks);
        assert_eq!(widths[0].len(), 2);
        assert_eq!(widths[0][0], [2, 2, 2, 2, 2, 2]); // 1+1 for each pair
        assert_eq!(widths[0][1], [5, 7, 9, 11, 13, 15]); // 2+3, 3+4, ...
    }
    ...
    fn test_normalized_widths_start_pattern() {
        // START_BLOCKS = [8,1,1,1,1,1,1,3]; element 7 (=3) is excluded.
        let blocks = vec![Vec::from(START_BLOCKS)];
        let widths = compute_normalized_widths(&blocks);
    ...
    fn test_normalized_widths_stop_pattern() {
        // STOP_BLOCKS = [7,1,1,3,1,1,1,2]; element 7 (=2) is excluded.
    ...
    // -------------------------------------------------------------------------
    // get_word_6 / width-word constants
    // -------------------------------------------------------------------------

    #[test]
    fn test_get_word_6_start_pattern() {
        // Start pattern [8,1,1,1,1,1,3] → widths [9,2,2,2,2,2] → base-13 little-endian
        let widths: [u32; 6] = [9, 2, 2, 2, 2, 2];
        assert_eq!(get_word_6(&widths), START_WORD_6);
    }

    #[test]
    fn test_get_word_6_stop_pattern() {
        // Stop pattern [7,1,1,3,1,1,1,2] → widths [8,2,4,4,2,2]
        let widths: [u32; 6] = [8, 2, 4, 4, 2, 2];
        assert_eq!(get_word_6(&widths), STOP_WORD_6);
    }

    #[test]
    fn test_get_word_6_max_width_no_collision() {
        ...
        assert_ne!(get_word_6(&a), get_word_6(&b));
        // Also check a 12 doesn't alias a two-digit base-10 sequence
        let c: [u32; 6] = [2, 1, 1, 1, 1, 1]; // would collide with [12,...] in base-10
        assert_ne!(get_word_6(&a), get_word_6(&c));
    }

    // -------------------------------------------------------------------------
    // decodes_exactly_6 / ext_codeword_of_width
    // -------------------------------------------------------------------------

    #[test]
    fn test_decodes_exactly_6_valid_codeword() {
        ...
        assert!(decodes_exactly_6(&widths));
    }

    #[test]
    fn test_decodes_exactly_6_all_zero() {
        assert!(!decodes_exactly_6(&[0u32; 6]));
    }

    #[test]
    fn test_decodes_exactly_6_invalid_pattern() {
        // [1,1,1,1,1,1] is not a valid PDF417 width pattern
        assert!(!decodes_exactly_6(&[1u32; 6]));
    }

    #[test]
    fn test_ext_codeword_of_width_start() {
        assert_eq!(ext_codeword_of_width(START_WORD_6), DUMMY_CW);
    }

    #[test]
    fn test_ext_codeword_of_width_stop() {
        assert_eq!(ext_codeword_of_width(STOP_WORD_6), DUMMY_CW);
    }
    ...
    // -------------------------------------------------------------------------
    // compute_garbage_word_witnesses_6
    // -------------------------------------------------------------------------

    #[test]
    fn test_garbage_witnesses_6_zero_row_picks_col_0() {
        // All-zero widths: get_word_6 returns 0 (not a valid word) → pick column 0.
        let row = vec![[0u32; 6]; 4];
        let (words, indices) = compute_garbage_word_witnesses_6(&[row]);
        assert_eq!(words, vec![0u64]);
        assert_eq!(indices, vec![0u64]);
    }

    #[test]
    fn test_garbage_witnesses_6_skips_start() {
    ...
    #[test]
    fn test_words_with_dummies_6_duplicate_row_zeroed() {
        // Two identical rows of all-zero widths: second row should be zeroed as duplicate.
        let row = vec![[0u32; 6]; 3];
        let (wwd, _garbage) = compute_words_with_dummies_6(&[row.clone(), row]);
    ...
    #[test]
    fn test_words_with_dummies_6_start_kept() {
        // START widths should be kept (not zeroed) in words_with_dummies.
        let start_widths: [u32; 6] = [9, 2, 2, 2, 2, 2];
        let zero = [0u32; 6];
        let row = vec![start_widths, zero, zero];
        let (wwd, _) = compute_words_with_dummies_6(&[row]);
        assert_eq!(wwd[0][0], START_WORD_6);
