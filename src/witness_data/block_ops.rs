...
mod tests {
    ...
    fn test_normalized_blocks_zero_window_filtered() {
        // A window with any zero element is replaced with all-zeros (not dropped).
        let blocks = vec![vec![3u32, 0, 2, 4, 1, 3, 2, 1]];
        let normalized = compute_normalized_blocks(&blocks);
        assert_eq!(normalized[0].len(), 1);
        assert_eq!(normalized[0][0], [0u32; 8]);
    }
    ...
    fn test_normalized_blocks_multiple_rows() {
        ...
        assert_eq!(normalized[0].len(), 1);
        assert_eq!(normalized[1].len(), 1); // zero window kept as [0u32; 8]
        assert_eq!(normalized[1][0], [0u32; 8]);
    ...
    fn test_normalized_blocks_mixed_windows() {
        ...
        let normalized = compute_normalized_blocks(&blocks);
        assert_eq!(normalized[0].len(), 2); // both windows kept; zero window is all-zeros
        assert_eq!(normalized[0][0], [0u32; 8]); // window 0 has a zero → all-zeros
        assert_eq!(normalized[0][1].iter().sum::<u32>(), 17);
    }
